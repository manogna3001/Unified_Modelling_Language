...
app.post('/unsubscribe/:topic', async (req, res) => {
    ...
    try {
        ...
        try {
                ...
                body: {
                    query: {
                        bool: {
                            must: [
                                { term: { username: username } },
                                { term: { category: topic } }  // Assuming 'category' is where the topic is stored
                            ]
                        }
                    }
                }
            ...
            console.log("Notification Delete Elasticsearch response:", JSON.stringify(notificationDeleteResponse, null, 2)); // Log the full response
...
app.get('/subscriptions/:username', async (req, res) => {
    ...
    try {
        ...
        if (!response) {
            console.error("Elasticsearch response is undefined!");
            return res.status(500).json({ message: "Elasticsearch response is undefined!" });
        }

        if (!response.hits) {
            console.error("Elasticsearch response hits is undefined!");
            return res.status(500).json({ message: "Elasticsearch response hits is undefined!" });
        }

        if (!response.hits.hits) {
            console.error("Elasticsearch response hits.hits is undefined!");
            return res.status(500).json({ message: "Elasticsearch response hits.hits is undefined!" });
        }
        ...
        console.error("Elasticsearch Error Details:", err.meta?.body ? JSON.stringify(err.meta.body, null, 2) : err);
...
app.get('/notifications/:username', async (req, res) => {
    ...
    try {
        ...
        const hits = response.hits.hits;
        const notifications = hits.map(hit => hit._source);
